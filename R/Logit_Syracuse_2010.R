...
colnames(ivs_Hausman_LA) <- c("Brand",
                              ...
                              "z3_P_Houston", "z4_P_Spokane", "z5_P_CHICAGO",
                              ...
                              "z4_log_P_Spokane", "z5_log_P_CHICAGO"
)

constant <- data.frame(constant=
                         matrix(
                           rep(1,
                               nrow(D8.2_2010_aggregate_data.SYRACUSE[[1]])),
                           ncol=1
                         )
)

scale_p2_Wmean <- scale(D8.2_2010_aggregate_data.SYRACUSE[[1]]["p2_Wmean"], center = FALSE, scale = apply(D8.2_2010_aggregate_data.SYRACUSE[[1]]["p2_Wmean"], 2, sd, na.rm = TRUE)); colnames(scale_p2_Wmean) <- "scale_p2_Wmean"

log_p2_Wmean <- data.frame(log_p2_Wmean=log(D8.2_2010_aggregate_data.SYRACUSE[[1]][, "p2_Wmean"]))

beerSyracuse.data <- cbind(constant, D8.2_2010_aggregate_data.SYRACUSE[[1]][c(1, 7,
                                                                       8)])
firm.id <- data.frame(firm.id=as.numeric(factor(beerSyracuse.data$Conglomerate)))

id <- 1000:(1000+nrow(beerSyracuse.data)-1)

beerSyracuse.data <- cbind(beerSyracuse.data, firm.id, id)

beerSyracuse.data <- cbind(beerSyracuse.data, scale_p2_Wmean, log_p2_Wmean)

beerSyracuse.data <- cbind(beerSyracuse.data,
                     D8.2_2010_aggregate_data.SYRACUSE[[1]][c(12, 14, 16,
                                                             33:82)])

tmp_beerSyracuse.data <- D8.2_2010_aggregate_data.SYRACUSE[[1]][, c(13, 17, 18, 19,
                                                             21:24)]

beerSyracuse.data <- cbind(beerSyracuse.data, tmp_beerSyracuse.data)

BLP_ivs <- log(hdm:::constructIV(beerSyracuse.data$firm.id, beerSyracuse.data$cdid,
                                 beerSyracuse.data$id, cbind(1, beerSyracuse.data[, c("ABV", "Calories_oz", "Carbs_oz")])))


beerSyracuse.data <- cbind(beerSyracuse.data, ivs_Hausman_LA[,2:16], BLP_ivs)


rm(constant, ivs_Hausman_LA, tmp_beerSyracuse.data, scale_p2_Wmean,
   log_p2_Wmean, BLP_ivs, firm.id, id)

saveRDS(beerSyracuse.data,
        paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/beerSyracuse.data.rds", sep = ""))

summary(lm.simple.logit.Syracuse <- lm(log(share) - log(outshr) ~ 1 +
                            p2_Wmean +
                            ABV +
                            prct_PR +
                            Calories_oz +
...
#iv_lm.ivs <- paste(colnames(beerSyracuse.data[c(72, 73, 75)]), collapse=" + ")
...
summary( iv.simple.logit.Syracuse <- ivreg( model.iv_lm.Brands,
                                    data= beerSyracuse.data))
