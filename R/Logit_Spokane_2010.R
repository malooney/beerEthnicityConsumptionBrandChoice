...
colnames(ivs_Hausman_LA) <- c("Brand",
                              ...
                              "z3_P_Houston", "z4_P_SYRACUSE", "z5_P_CHICAGO",
                              "z1_scale_P_LOSANGELES", "z2_scale_P_DallasTx",
                              "z3_scale_P_Houston", "z4_scale_P_SYRACUSEe",
...
constant <- data.frame(constant=
                         matrix(
                           rep(1,
                               nrow(D8.2_2010_aggregate_data.SPOKANE[[1]])),
                           ncol=1
                         )
)

scale_p2_Wmean <- scale(D8.2_2010_aggregate_data.SPOKANE[[1]]["p2_Wmean"], center = FALSE, scale = apply(D8.2_2010_aggregate_data.SPOKANE[[1]]["p2_Wmean"], 2, sd, na.rm = TRUE)); colnames(scale_p2_Wmean) <- "scale_p2_Wmean"

log_p2_Wmean <- data.frame(log_p2_Wmean=log(D8.2_2010_aggregate_data.SPOKANE[[1]][, "p2_Wmean"]))

beerSpokane.data <- cbind(constant, D8.2_2010_aggregate_data.SPOKANE[[1]][c(1, 7,
                                                                        8)])
firm.id <- data.frame(firm.id=as.numeric(factor(beerSpokane.data$Conglomerate)))

id <- 1000:(1000+nrow(beerSpokane.data)-1)

beerSpokane.data <- cbind(beerSpokane.data, firm.id, id)

beerSpokane.data <- cbind(beerSpokane.data, scale_p2_Wmean, log_p2_Wmean)

beerSpokane.data <- cbind(beerSpokane.data,
                     D8.2_2010_aggregate_data.SPOKANE[[1]][c(12, 14, 16,
                                                              33:82)])

tmp_beerSpokane.data <- D8.2_2010_aggregate_data.SPOKANE[[1]][, c(13, 17, 18, 19,
                                                              21:24)]

beerSpokane.data <- cbind(beerSpokane.data, tmp_beerSpokane.data)

BLP_ivs <- log(hdm:::constructIV(beerSpokane.data$firm.id, beerSpokane.data$cdid,
                                 beerSpokane.data$id, cbind(1, beerSpokane.data[, c("ABV", "Calories_oz", "Carbs_oz")])))


beerSpokane.data <- cbind(beerSpokane.data, ivs_Hausman_LA[,2:16], BLP_ivs)

rm(constant, ivs_Hausman_LA, tmp_beerSpokane.data, scale_p2_Wmean, log_p2_Wmean,
   BLP_ivs, firm.id, id)

saveRDS(beerSpokane.data,
        paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/beerSpokane.data.rds", sep = ""))

summary(lm.simple.logit.Spokane <- lm(log(share) - log(outshr) ~ 1 +
                            p2_Wmean +
                            ABV +
                            prct_PR +
                            Calories_oz +
...
#iv_lm.ivs <- paste(colnames(beerSpokane.data[c(72, 73, 75)]), collapse=" + ")
...
summary( iv.simple.logit.Spokane <- ivreg( model.iv_lm.Brands,
                                    data= beerSpokane.data))
