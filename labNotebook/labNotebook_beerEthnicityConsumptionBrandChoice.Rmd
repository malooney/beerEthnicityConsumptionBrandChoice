...
hora <- S8.2.generate_aggregate_data_frame(nWeeks = 52,
                                         nmkt = 52,
                                         city="CHICAGO",
...
# Load Data ------------------------------------------------------------------
  ...
  if(!file.exists(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.CHICAGO.rds", sep=""))) {

    stop("file does not exist in project directory.")

  } else{

  D8.2_2010_aggregate_data.CHICAGO  <- readRDS(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.CHICAGO.rds", sep=""))

  }

  lm.Brands <- paste(colnames(D8.2_2010_aggregate_data.CHICAGO[[1]][33:52]), collapse=" + ")

  model.lm.Brands <- formula(paste("log(share) - log(outshr)~  0 + 
                                   p2_Wmean +", lm.Brands, sep=""))

  summary(lm.results <- lm( model.lm.Brands,
                          data= D8.2_2010_aggregate_data.CHICAGO[[1]]))
...
stargazer::stargazer(D8.2_2010_aggregate_data.CHICAGO[[1]][,10:19],
          summary=T,
          type="latex",
          title=" Summary Statistics: CHICAGO")
...
# Load Data -------------------------------------------------------------------
  ...
  if(!file.exists(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.DALLASTX.rds", sep=""))) {

    stop("file does not exist in project directory.")

  } else{

  D8.2_2010_aggregate_data.DALLASTX  <- readRDS(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.DALLASTX.rds", sep=""))

  }

  lm.Brands <- paste(colnames(D8.2_2010_aggregate_data.DALLASTX[[1]][33:52]), collapse=" + ")

  model.lm.Brands <- formula(paste("log(share) - log(outshr)~  0 + 
                                   p2_Wmean +", lm.Brands, sep=""))

  summary(lm.results <- lm( model.lm.Brands,
                          data= D8.2_2010_aggregate_data.DALLASTX[[1]]))
...
stargazer::stargazer(D8.2_2010_aggregate_data.DALLASTX[[1]][,10:19],
          summary=T,
          type="latex",
          title=" Summary Statistics: DALLAS, TX")
...
# Load Data -------------------------------------------------------------------
  ...
  if(!file.exists(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.LOSANGELES.rds", sep=""))) {

    stop("file does not exist in project directory.")

  } else{

  D8.2_2010_aggregate_data.LOSANGELES  <- readRDS(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.LOSANGELES.rds", sep=""))

  }

  lm.Brands <- paste(colnames(D8.2_2010_aggregate_data.LOSANGELES[[1]][33:52]), collapse=" + ")

  model.lm.Brands <- formula(paste("log(share) - log(outshr)~  0 + 
                                   p2_Wmean +", lm.Brands, sep=""))

  summary(lm.results <- lm( model.lm.Brands,
                          data= D8.2_2010_aggregate_data.LOSANGELES[[1]]))
...
stargazer::stargazer(D8.2_2010_aggregate_data.LOSANGELES[[1]][,10:19],
          summary=T,
          type="latex",
...
# Load Data -------------------------------------------------------------------
  ...
  if(!file.exists(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.SPOKANE.rds", sep=""))) {

    stop("file does not exist in project directory.")

  } else{

  D8.2_2010_aggregate_data.SPOKANE  <- readRDS(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.SPOKANE.rds", sep=""))

  }

  lm.Brands <- paste(colnames(D8.2_2010_aggregate_data.SPOKANE[[1]][33:52]), collapse=" + ")

  model.lm.Brands <- formula(paste("log(share) - log(outshr)~  0 + 
                                   p2_Wmean +", lm.Brands, sep=""))

  summary(lm.results <- lm( model.lm.Brands,
                          data= D8.2_2010_aggregate_data.SPOKANE[[1]]))
...
stargazer::stargazer(D8.2_2010_aggregate_data.SPOKANE[[1]][,10:19],
          summary=T,
          type="latex",
...
# Load Data -------------------------------------------------------------------
  ...
  if(!file.exists(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.SYRACUSE.rds", sep=""))) {

    stop("file does not exist in project directory.")

  } else{

  D8.2_2010_aggregate_data.SYRACUSE  <- readRDS(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.SYRACUSE.rds", sep=""))

  }

  lm.Brands <- paste(colnames(D8.2_2010_aggregate_data.SYRACUSE[[1]][33:52]), collapse=" + ")

  model.lm.Brands <- formula(paste("log(share) - log(outshr)~  0 + 
                                   p2_Wmean +", lm.Brands, sep=""))

  summary(lm.results <- lm( model.lm.Brands,
                          data= D8.2_2010_aggregate_data.SYRACUSE[[1]]))
...
stargazer::stargazer(D8.2_2010_aggregate_data.SPOKANE[[1]][,10:19],
          summary=T,
          type="latex",
          title=" Summary Statistics: SYRACUSE")
...
# Load Data -------------------------------------------------------------------
  ...
  if(!file.exists(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.HOUSTON.rds", sep=""))) {

    stop("file does not exist in project directory.")

  } else{

  D8.2_2010_aggregate_data.HOUSTON  <- readRDS(paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/D8.2_2010_aggregate_data.HOUSTON.rds", sep=""))

  }

  lm.Brands <- paste(colnames(D8.2_2010_aggregate_data.HOUSTON[[1]][33:52]), collapse=" + ")

  model.lm.Brands <- formula(paste("log(share) - log(outshr)~  0 + 
                                   p2_Wmean +", lm.Brands, sep=""))

  summary(lm.results <- lm( model.lm.Brands,
                          data= D8.2_2010_aggregate_data.HOUSTON[[1]]))
...
stargazer::stargazer(D8.2_2010_aggregate_data.HOUSTON[[1]][,10:19],
          summary=T,
          type="latex",
          title=" Summary Statistics: HOUSTON")

```

```{r, echo=FALSE, cache=F, message=F, warning=F}

plot_Hausman_IV <- function(brand= "BUD LIGHT"){
...
budLight_Chicago <- dplyr::filter(D8.2_2010_aggregate_data.CHICAGO[[1]],
                           Brand==brand)

budLight_DallasTx <- dplyr::filter(D8.2_2010_aggregate_data.DALLASTX[[1]],
                           Brand==brand)

budLight_Houston <- dplyr::filter(D8.2_2010_aggregate_data.HOUSTON[[1]],
                           Brand==brand)

budLight_LosAngeles <- dplyr::filter(D8.2_2010_aggregate_data.LOSANGELES[[1]],
                           Brand==brand)

budLight_Spokane <- dplyr::filter(D8.2_2010_aggregate_data.SPOKANE[[1]],
                           Brand==brand)

budLight_Syracuse <- dplyr::filter(D8.2_2010_aggregate_data.SYRACUSE[[1]],
                           Brand==brand)

par(mfrow= c(1, 1), ps= 10, cex= 0.75, cex.main= 1.25)

plot(budLight_Chicago$p2_Wmean_non_pr,
     type="l",
     col="black",
     ylim=c(0, 20), 
     xlab = "Week (1:52)", 
     ylab = paste(brand, "($/Gallon)", sep=""), 
     main = paste("Price of ", brand, " across Cities \n Non-price reduced", sep=""), 
     sub="")

lines(budLight_DallasTx$p2_Wmean_non_pr, type="l", col="blue")
lines(budLight_Houston$p2_Wmean_non_pr, type="l", col="red")
lines(budLight_LosAngeles$p2_Wmean_non_pr, type="l", col="green")
lines(budLight_Spokane$p2_Wmean_non_pr, type="l", col="gold")
lines(budLight_Syracuse$p2_Wmean_non_pr, type="l", col="purple")

legend("bottomright",  c("Chicago", "Dallas", "Houston", "Los Angeles", 
                         "Spokane", "Syracuse"), 
       lty= c(1), col= c("black", "blue", "red", "green", "gold", "purple"))
}
