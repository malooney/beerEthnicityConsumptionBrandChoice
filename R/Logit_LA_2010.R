...
scale_p2_Wmean <- scale(D8.2_2010_aggregate_data.LOSANGELES[[1]]["p2_Wmean"], center = FALSE, scale = apply(D8.2_2010_aggregate_data.LOSANGELES[[1]]["p2_Wmean"], 2, sd, na.rm = TRUE)); colnames(scale_p2_Wmean) <- "scale_p2_Wmean"

log_p2_Wmean <- data.frame(log_p2_Wmean=log(D8.2_2010_aggregate_data.LOSANGELES[[1]][, "p2_Wmean"]))

beerLA.data <- cbind(constant, D8.2_2010_aggregate_data.LOSANGELES[[1]][c(1, 7,
                                                                          8)])
...
beerLA.data <- cbind(beerLA.data, scale_p2_Wmean, log_p2_Wmean)

beerLA.data <- cbind(beerLA.data,
                     D8.2_2010_aggregate_data.LOSANGELES[[1]][c(12, 14, 16,
                                                                33:82)])
...
BLP_ivs <- log(hdm:::constructIV(beerLA.data$firm.id, beerLA.data$cdid,
                                 beerLA.data$id, cbind(1, beerLA.data[, c("ABV", "Calories_oz", "Carbs_oz")])))


beerLA.data <- cbind(beerLA.data, ivs_Hausman_LA[,2:16], BLP_ivs)

rm(constant, ivs_Hausman_LA, tmp_beerLA.data, scale_p2_Wmean, log_p2_Wmean,
   BLP_ivs, firm.id, id)

saveRDS(beerLA.data,
        paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/beerLA.data.rds", sep = ""))



# simple logit regressions -----------------------------------------------------

# lm.Brands <- paste(colnames(beerLA.data[12:62]), collapse = " + ")
#
...
# Minimum Distance Estimator (MDE) --------------------------------------------
  # mde <- function(){
  #   omega = solve(vcov(lm.results)[2:21,2:21],
...
summary(lm.simple.logit.LA <- lm(log(share) - log(outshr) ~ 1 +
                            p2_Wmean +
                            ABV +
                            prct_PR +
                            Calories_oz +
...
# iv_lm.Brands <- paste(colnames(beerLA.data[12:62]), collapse=" + ")
#
# iv_lm.ivs <- paste(colnames(beerLA.data[c(72, 73, 75)]), collapse=" + ")
# #iv_lm.ivs <- paste(colnames(beerLA.data[c(71:75)]), collapse=" + ")
#
...
iv_lm.char <- paste(colnames(beerLA.data[c(62:65, 68:69)]), collapse=" + ")
...
summary( iv.simple.logit.LA <- ivreg( model.iv_lm.Brands,
                                    data= beerLA.data))
