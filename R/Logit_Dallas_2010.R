...
constant <- data.frame(constant=
                         matrix(
                           rep(1,
                               nrow(D8.2_2010_aggregate_data.DALLASTX[[1]])),
                           ncol=1
                         )
)

scale_p2_Wmean <- scale(D8.2_2010_aggregate_data.DALLASTX[[1]]["p2_Wmean"], center = FALSE, scale = apply(D8.2_2010_aggregate_data.DALLASTX[[1]]["p2_Wmean"], 2, sd, na.rm = TRUE)); colnames(scale_p2_Wmean) <- "scale_p2_Wmean"

log_p2_Wmean <- data.frame(log_p2_Wmean=log(D8.2_2010_aggregate_data.DALLASTX[[1]][, "p2_Wmean"]))

beerDallas.data <- cbind(constant, D8.2_2010_aggregate_data.DALLASTX[[1]][c(1, 7,
                                                                       8)])
firm.id <- data.frame(firm.id=as.numeric(factor(beerDallas.data$Conglomerate)))

id <- 1000:(1000+nrow(beerDallas.data)-1)

beerDallas.data <- cbind(beerDallas.data, firm.id, id)

beerDallas.data <- cbind(beerDallas.data, scale_p2_Wmean, log_p2_Wmean)

beerDallas.data <- cbind(beerDallas.data,
                     D8.2_2010_aggregate_data.DALLASTX[[1]][c(12, 14, 16,
                                                             33:82)])

tmp_beerDallas.data <- D8.2_2010_aggregate_data.DALLASTX[[1]][, c(13, 17, 18, 19,
                                                             21:24)]

beerDallas.data <- cbind(beerDallas.data, tmp_beerDallas.data)

BLP_ivs <- log(hdm:::constructIV(beerDallas.data$firm.id, beerDallas.data$cdid,
                                 beerDallas.data$id, cbind(1, beerDallas.data[, c("ABV", "Calories_oz", "Carbs_oz")])))


beerDallas.data <- cbind(beerDallas.data, ivs_Hausman_LA[,2:16], BLP_ivs)

rm(constant, ivs_Hausman_LA, tmp_beerDallas.data, scale_p2_Wmean, log_p2_Wmean,
   BLP_ivs, firm.id, id)

saveRDS(beerDallas.data,
        paste(path.local, "/data_beerEthnicityConsumptionBrandChoice/beerDallas.data.rds", sep = ""))

summary( lm.simple.Logit.Dallas <- lm(log(share) - log(outshr) ~ 1 +
                            p2_Wmean +
                            ABV +
                            prct_PR +
                            Calories_oz +
...
#iv_lm.ivs <- paste(colnames(beerDallas.data[c(72, 73, 75)]), collapse=" + ")
...
summary( iv.simple.logit.Dallas <- AER::ivreg( model.iv_lm.Brands,
                                    data= beerDallas.data))
